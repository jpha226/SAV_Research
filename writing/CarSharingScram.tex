...
\pdfinfo{
/Title (Minimum Cost Matching for an Autonomous Car Sharing System)
/Author (Anonymous)}
...
A drawback of the presented centralized systems is that users can improve their wait time by misreporting their location.

\begin{figure}
\centering
\begin{tikzpicture}
\centering
\draw[step=0.5cm,color=gray] (-1.0,-1.0) grid (4.0,0.5);
\node at (-0.75,+0.25) {C1};
\node at (-1.25,+0.75) {T1};
\node at (3.75,0.25) {C2};
%\draw (-0.25,-0.75) ellipse (8mm and 3mm);
