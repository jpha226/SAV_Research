...
\pdfinfo{
/Title (Minimum Cost Matching for an Autonomous Carsharing System)
/Author (Anonymous)}
...
\begin{figure}
\begin{algorithmic}
\Function{Decentralized}{$C,T$}

\While{$\exists t \in T$ s.t. t.hasLooked == FALSE}
...
\begin{figure}
\begin{algorithmic}
\Function{GreedyCentralized}{$C,T,maxRadius$}
\State $radius = 0$
\For{$radius \leq maxRadius$}
\ForAll{$t \in T$}
...
%	ylabel=Average Unoccupied Distance Traveled (miles per day),
...
%	coordinates {(Decentralized,4.12657) += (0,0.0256335429) -= (0,0.0256335429) (Greedy,3.421545178) += (0,0.0118211151) -= (0,0.0118211151) 
%		 (Hungarian,1.98288595) += (0,0.0064259937) -= (0,0.0064259937) (SCRAM,1.6660924676) += (0,0.0061273067) -= (0,0.0061273067)};
%\end{axis}
%\end{tikzpicture}
%}
...
%\begin{axis}[
%	x tick label style={
%	/pgf/number format/1000 sep=0pt anchor=east shift=-4pt},
...
\centering
\begin{tikzpicture}
\draw[step=0.5cm,color=gray] (-1.0,-2.0) grid (4.5,0.5);
\node at (-0.75,+0.25) {C1};
\node at (1.25,-1.25) {T1};
\node at (1.75,0.25) {C2};
%\draw (-0.25,-0.75) ellipse (8mm and 3mm);
...
\node at (1.75,-1.25) {C2};
%\draw (-0.25,-0.75) ellipse (8mm and 3mm);
\draw[dashed,thick,->] (-0.75,0.25) -- (0.5,0.25);
\draw[dashed,thick,->] (1.75,0.25) -- (1.75,-1.0);
\node at (+4.25,0.25) {T2};
\node at (1.75,-1.75) {X};
\end{tikzpicture}
\end{subfigure}
